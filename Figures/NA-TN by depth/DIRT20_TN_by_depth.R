...
#Changing control C values for 0-10 cm
...
soc_summary <- data.raw %>% group_by(TRT, Depth) %>% summarize(n = n(),
                                                      mean = mean(bulk_percN*10),
                                                      stdev = sd(bulk_percN*10),
                                                      sterr = sd(bulk_percN*10)/sqrt(3))

#save summary
write.csv(soc_summary,"DIRT20_TN by depth_treatment summary.csv")
...
ggsave(plot=f1_all, filename = "DIRT20_soilTN_all_trts.png",
